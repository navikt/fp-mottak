...
import no.nav.foreldrepenger.mottak.leesah.pdl.oppslag.ForeldreTjeneste;
import no.nav.foreldrepenger.mottak.leesah.pdl.tjeneste.DateUtil;
import no.nav.foreldrepenger.mottak.leesah.pdl.tjeneste.ForsinkelseKonfig;
...
class VurderSorteringTaskTest {
    ...
    void skal_berike_og_grovsortere_fødselshendelse_som_er_klar() {
        ...
        // Assert
        var beriketHendelse = hendelseRepository.finnEksaktHendelse(inngåendeHendelse.getHendelseId());
    ...
    void skal_opprette_ny_vurder_sortering_task_for_fødselshendelse_som_ikke_er_klar() {
        ...
        var hendelse = hendelseRepository.finnEksaktHendelse(inngåendeHendelse.getHendelseId());
    ...
    void skal_ikke_grovsortere_fødselshendelsesom_er_merket_håndtert() {
        ...
        // Assert
        var hendelse = hendelseRepository.finnEksaktHendelse(hendelseOpprettet.getHendelseId());
        assertThat(hendelse.getHåndtertStatus()).isEqualTo(HåndtertStatusType.HÅNDTERT);
    ...
    void skal_sette_alle_håndtert_når_det_finnes_en_senere_lenket_annulert() {
        ...
        // Assert
        var hendelseA = hendelseRepository.finnEksaktHendelse(hendelseOpprettet.getHendelseId());
        assertThat(hendelseA.getHåndtertStatus()).isEqualTo(HåndtertStatusType.HÅNDTERT);
        var hendelseB = hendelseRepository.finnEksaktHendelse(hendelseAnnullert.getHendelseId());
    ...
    void skal_grovsortere_korrigering_da_en_tidligere_sendt_fødselshendelse_hadde_forskjellig_fødselsdato() {
        ...
        // Assert
        var beriketHendelse = hendelseRepository.finnEksaktHendelse(hendelseKorrigert2.getHendelseId());
        assertThat(beriketHendelse.getHåndtertStatus()).isEqualTo(HåndtertStatusType.SENDT_TIL_SORTERING);
        var beriketFødsel = DefaultJsonMapper.fromJson(beriketHendelse.getPayload(), PdlFødsel.class);
        assertThat(beriketFødsel.getAktørIdForeldre()).containsExactlyInAnyOrder(AKTØR_ID_MOR, AKTØR_ID_FAR);
    ...
    void skal_ikke_grovsortere_korrigering_da_en_tidligere_sendt_fødselshendelse_har_samme_fødselsdato() {
        ...
        // Assert
        var hendelse = hendelseRepository.finnEksaktHendelse(hendelseKorrigert.getHendelseId());
        assertThat(hendelse.getHåndtertStatus()).isEqualTo(HåndtertStatusType.HÅNDTERT);
    ...
    void skal_grovsortere_korrigering_der_tidligere_fødselshendelser_ikke_ble_sendt_til_fpsak() {
        ...
        // Assert
        var hendelse = hendelseRepository.finnEksaktHendelse(hendelseKorrigert2.getHendelseId());
        assertThat(hendelse.getHåndtertStatus()).isEqualTo(HåndtertStatusType.MOTTATT);
