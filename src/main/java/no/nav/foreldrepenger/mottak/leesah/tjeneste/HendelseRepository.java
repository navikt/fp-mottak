...
public class HendelseRepository {
    ...
    public InngåendeHendelse finnEksaktHendelse(String inngåendeHendelseId) {
    ...
    public Optional<InngåendeHendelse> finnHendelseFraIdHvisFinnes(String hendelseId) {
        TypedQuery<InngåendeHendelse> query = entityManager.createQuery("from InngåendeHendelse where hendelseId = :hendelseId ", InngåendeHendelse.class);
        query.setParameter(HENDELSE_ID, hendelseId);
        return queryTilOptional(hendelseId, query);
    }
    ...
    public Optional<InngåendeHendelse> finnGrovsortertHendelse(String hendelseId) {
        TypedQuery<InngåendeHendelse> query = entityManager.createQuery("from InngåendeHendelse where hendelseId = :hendelseId " +
            "and håndtertStatus = :håndtertStatus " +
            SORTER_STIGENDE_PÅ_OPPRETTET_TIDSPUNKT, InngåendeHendelse.class);
        query.setParameter(HENDELSE_ID, hendelseId);
        ...
        if (resultater.size() > 1) {
            LOGGER.warn(new TekniskException("FP-164339",
                String.format("Fant mer enn en InngåendeHendelse med hendelseId=%s og håndtertStatus=%s.",
                    hendelseId, HåndtertStatusType.GROVSORTERT))
                .getMessage());
        } else if (resultater.isEmpty()) {
            LOGGER.warn(new TekniskException("FP-264339",
                String.format("Fant ikke InngåendeHendelse med hendelseId=%s og håndtertStatus=%s.",
