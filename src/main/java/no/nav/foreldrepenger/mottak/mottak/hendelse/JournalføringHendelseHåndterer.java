...
public class JournalføringHendelseHåndterer implements KafkaMessageHandler<String, JournalfoeringHendelseRecord> {
    ...
    void handleMessage(JournalfoeringHendelseRecord payload) {
        ...
        if (HENDELSE_ENDRET.equalsIgnoreCase(payload.getHendelsesType())) {
            ...
            LOG.info("FPMOTTAK Tema Endret fra {} journalpost {} kanal {} referanse {}", gammeltTema, arkivId, mottaksKanal, eksternReferanseId);
        }

        // EESSI har egen mottaksprosess m/BEH_SED-oppgaver.
        if (EESSI.equals(mottaksKanal)) {
            LOG.info("FPMOTTAK Mottatt Journalføringhendelse ignorerer journalpost {} kanal {}", arkivId, mottaksKanal);
            return;
        }

        LOG.info("FPMOTTAK Mottatt Journalføringhendelse type {} journalpost {} referanse {}", hendelseType, arkivId, eksternReferanseId);
