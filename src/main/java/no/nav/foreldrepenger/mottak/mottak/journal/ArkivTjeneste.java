package no.nav.foreldrepenger.mottak.mottak.journal;
...
import no.nav.foreldrepenger.mottak.mottak.journal.saf.SafTjeneste;
...
public class ArkivTjeneste {
    ...
    private static final String KUNNE_IKKE_OPPDATERE_JP = "FPMOTTAK Kunne ikke oppdatere journalpost ";
    ...
    public Optional<String> hentEksternReferanseId(Journalpost journalpost) {
        ...
        if (LOG.isInfoEnabled()) {
            LOG.info("FPMOTTAK hentEksternReferanseId fant referanse {} for journalpost {}", loggtekst, journalpost.journalpostId());
        }
        return referanse;
    }
    ...
    public void settTilleggsOpplysninger(ArkivJournalpost arkivJournalpost, DokumentTypeId defaultDokumentTypeId, boolean manuellJournalføring) {
        ...
        if (tilleggDoktype.isEmpty() || tilleggDoktype.stream().noneMatch(to -> hovedtype.getOffisiellKode().equals(to.verdi()))) {
            var builder = OppdaterJournalpostRequest.ny();
            if (LOG.isInfoEnabled()) {
                LOG.info("FPMOTTAK oppdaterer/legger til tilleggsopplysninger for {} med {}", journalpost.journalpostId(), hovedtype.getOffisiellKode());
            }
    ...
    public boolean oppdaterRettMangler(ArkivJournalpost arkivJournalpost,
        ...
        if ((journalpost.avsenderMottaker() == null) || (journalpost.avsenderMottaker().id() == null) || (journalpost.avsenderMottaker().navn()
            ...
            if (LOG.isInfoEnabled()) {
                LOG.info("FPMOTTAK oppdaterer manglende avsender for {}", journalpost.journalpostId());
            }
        ...
        if (journalpost.tittel() == null) {
            if (DokumentTypeId.UDEFINERT.equals(hovedtype)) {
                ...
                if (LOG.isInfoEnabled()) {
                    LOG.info("FPMOTTAK oppdaterer manglende tittel for {}", journalpost.journalpostId());
                }
        ...
        if (journalpost.tema() == null) {
            if (LOG.isInfoEnabled()) {
                LOG.info("FPMOTTAK oppdaterer manglende tema for {}", journalpost.journalpostId());
            }
        ...
        if ((journalpost.bruker() == null) || (journalpost.bruker().id() == null)) {
            if (LOG.isInfoEnabled()) {
                LOG.info("FPMOTTAK oppdaterer manglende bruker for {}", journalpost.journalpostId());
            }
        ...
        if (!oppdaterDok.isEmpty() && LOG.isInfoEnabled()) {
                LOG.info("FPMOTTAK oppdaterer manglende dokumenttitler for {}", journalpost.journalpostId());
        }
        ...
        if (!resultat && LOG.isInfoEnabled()) {
            LOG.info("FPMOTTAK oppdaterer gjenstår tittel eller dokumenttittel for {}", journalpost.journalpostId());
        }
        return resultat;
    }
    ...
    public void oppdaterJournalpostVedManuellJournalføring(String journalpostId, String nyJournalpostTittel, List<OppdaterJournalpostRequest.DokumentInfoOppdater> dokumenter,
        ...
        if ((originalJournalpost.avsenderMottaker() == null) || (originalJournalpost.avsenderMottaker().id() == null) || (originalJournalpost.avsenderMottaker().navn() == null)) {
            ...
            var navn = personTjeneste.hentNavn(utledetBehandlingTema, aktørId);
            if (LOG.isInfoEnabled()) {
                LOG.info("FPMOTTAK RESTJOURNALFØRING: oppdaterer manglende avsender for {}", originalJournalpost.journalpostId());
            }
        ...
        if (originalJournalpost.tema() == null) {
            if (LOG.isInfoEnabled()) {
                LOG.info("FPMOTTAK RESTJOURNALFØRING: oppdaterer manglende tema for {}", originalJournalpost.journalpostId());
            }
        ...
        if (originalJournalpost.tittel() == null && nyJournalpostTittel == null) {
            throw new IllegalStateException("FPMOTTAK RESTJOURNALFØRING: Kan ikke ferdigstille journalpost uten tittel på journalpost");
        }

        if (nyJournalpostTittel != null) {
            LOG.info("FPMOTTAK RESTJOURNALFØRING: Legger på ny journalpostTittel:{} for journalpostId:{} ", nyJournalpostTittel, journalpostId);
            oppdaterJournalpostBuilder.medTittel(nyJournalpostTittel);
        }

        if (!dokumenter.isEmpty()) {
            dokumenter.forEach( dok-> {
                LOG.info("FPMOTTAK RESTJOURNALFØRING:Legger på tittel:{} for dokumentId:{} for journalspostId:{} ", dok.tittel(), dok.dokumentInfoId(), journalpostId);
    ...
    public void oppdaterMedSak(String journalpostId, String sakId, String aktørId) {
        if (sakId == null) {
            throw new IllegalArgumentException("FPMOTTAK oppdaterMedSak mangler saksnummer " + journalpostId);
        }
        ...
        if (dokArkivTjeneste.oppdaterJournalpost(journalpostId, builder.build())) {
            LOG.info("FPMOTTAK SAKSOPPDATERING oppdaterte {} med sak {}", journalpostId, sakId);
        } else {
            throw new IllegalStateException("FPMOTTAK Kunne ikke knytte journalpost " + journalpostId + " til sak " + sakId);
        }
    }
    ...
    public void oppdaterMedGenerellSak(String journalpostId, String aktørId) {
        ...
        if (dokArkivTjeneste.oppdaterJournalpost(journalpostId, builder.build())) {
            LOG.info("FPMOTTAK SAKSOPPDATERING oppdaterte {} med generell sak", journalpostId);
        } else {
            throw new IllegalStateException("FPMOTTAK Kunne ikke knytte journalpost " + journalpostId + " til generell sak");
        }
    }
    ...
    public void ferdigstillJournalføring(String journalpostId, String enhet) {
        if (dokArkivTjeneste.ferdigstillJournalpost(journalpostId, enhet)) {
            LOG.info("FPMOTTAK FERDIGSTILLING ferdigstilte journalpost {} enhet {}", journalpostId, enhet);
        } else {
            throw new IllegalStateException("FPMOTTAK Kunne ikke ferdigstille journalpost " + journalpostId);
        }
    }
    ...
    public String knyttTilAnnenSak(ArkivJournalpost journalpost, String enhet, String sakId, String aktørId) {
        ...
        if (resultat != null) {
            LOG.info("FPMOTTAK KNYTTILANNENSAK journalpost {} ny sak {} ny journalpost {} enhet {}", journalpost.getJournalpostId(), sakId, resultat.nyJournalpostId(), enhet);
            return resultat.nyJournalpostId();
        } else {
            throw new IllegalStateException("FPMOTTAK Kunne ikke knytte journalpost " + journalpost.getJournalpostId() + " til sak " + sakId);
        }
    }
